pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'minnudocker786/techsolutions-app'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/haseena786-786/microservices-ingress-kastro.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|minnudocker786/techsolutions-app:latest|kastrov/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
